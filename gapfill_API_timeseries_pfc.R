...
## Requires:
# API_mean = an N_countries (rows) x N_years (columns) matrix giving estimated mean APIs (units of per 1000 PYO)
# API_low = an N_countries (rows) x N_years (columns) matrix giving estimated lower bound on APIs (units of per 1000 PYO)
# API_high = an N_countries (rows) x N_years (columns) matrix giving estimated upper bound on APIs (units of per 1000 PYO)
# Covariates = an N_years x N_covariates x N_countries array giving covariates for modelling

# For testing function:
API_mean <- data.units.ts; API_low <- data.units.ts.low; API_high <- data.units.ts.high; covariates <- covariates

 # gapfill_API_timeseries_pfc <- function(API_mean,API_low,API_high,covariates) {

  # Catch basic input errors
  if (!(dim(API_mean)==dim(API_low) && dim(API_mean)==dim(API_high))) {stop("API_mean,API_low,API_high matrices not equal sizes.")}
  if (!prod(dim(API_mean)==dim(covariates)[c(3,1)])) {stop("Covariate array improper size given API inputs.")}
  if (!prod(!is.na(covariates))) {stop("Covariate array contains NAs.")}
  
  # # Standardise covariates (in case this has not been done prior to input)
  # for (i in 1:(dim(covariates)[2])) {
  #   covariates[,i,] <- (covariates[,i,]-mean(covariates[,i,]))/sd(covariates[,i,])
  # }
  
  # Prepare TMB input data list
  N_countries <- dim(API_mean)[1]
  N_years <- dim(API_mean)[2]
  ...
  API_sd <- matrix(-99,nrow=N_countries,ncol=N_years)
  API_sd[missing_data==0 & fixed_data==0] <- ((API_high-API_mean)/2+(API_mean-API_low)/2)[missing_data==0 & fixed_data==0]/diff(qnorm(c(0.025,0.9755)))
  ...
  parameters$log_national_scalefactor_noise <- rep(0,N_countries)

  parameters$national_log_initial_API_offsets <- rep(0,N_countries)
  parameters$national_shortterm_noise <- matrix(0,nrow=N_countries,ncol=N_years-1)

  compile("gapfill_API_timeseries_pfc.cpp") # Compile the C++ file.
  dyn.load(dynlib("gapfill_API_timeseries_pfc")) # Dynamically link the C++ code.
  ...
  obj <- MakeADFun(data,parameters,random=c('national_log_initial_API_offsets', 'national_shortterm_noise'),DLL="gapfill_API_timeseries_pfc")
  ...
  compile("gapfill_API_timeseries_fixedpar_pfc.cpp") # Compile the C++ file.
  
  dyn.load(dynlib("gapfill_API_timeseries_fixedpar_pfc")) 
  
  obj2 <- MakeADFun(data,parameters,random=c('national_log_initial_API_offsets', 'national_shortterm_noise'),DLL="gapfill_API_timeseries_fixedpar_pfc")
  ...
  API_timeseries_realisations <- array(NA,dim=c(N_countries,N_years,1000))
  for (i in 1:1000) {
    buffer <- obj2$report(r.draws[i,])$API_timeseries # Computes API_timeseries matrix based on the simulations and model (Monte Carlo, prior predictive distribution).
    buffer[fixed_data==1] <- API_mean[fixed_data==1] # Input fixed data.
    # for (j in 1:N_countries) {
# 	  if (isNotEmpty(API_mean[j,missing_data[j,]==0][length(which(missing_data[j,]==0))])){ # If we have a last API_mean available, i.e. we have data for this country. 
#         if (API_mean[j,missing_data[j,]==0][length(which(missing_data[j,]==0))]==0 & max(which(missing_data[j,]==0))<N_years) { 
...
# 	  } else buffer[j,] <- 0 # Have no data for this country.  
    # }
    API_timeseries_realisations[,,i] <- buffer 
    cat(i,"\n")
  }
