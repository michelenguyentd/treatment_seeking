...
config_file <- read.csv("Z:/Config_Data/National_Config_Data.csv", stringsAsFactors = FALSE )
...
config_file$MAP_Country_Name[config_file$ISO3 == "SWZ"] <- "Eswatini"
...
sub_config_file_2 <- read.csv("Z:/GBD2019/Processing/Config_Data/Combined_Config_Data.csv")

# Need a better way to determine which subnational units to model later...
subnational_API_data <- read.csv("Z:/GBD2019/Processing/Stages/04c_API_Data_Export/Checkpoint_Outputs/Decomp 4/subnational_ihme_only.csv")
new_sub_list <- unique(subnational_API_data[subnational_API_data$iso3 %in% c("PAK", "PHL"), c("country_name", "admin_unit_name", "ihme_id")])

# Add in Pakistan units not in subnational API dump:
if (!("Gilgit-Baltistan" %in% new_sub_list)){
  extra_row <- subnational_API_data[which(subnational_API_data$iso3 == "PAK")[1], ]
  config_row <- sub_config_file_2[sub_config_file_2$IHME_Location_ASCII_Name == "Gilgit-Baltistan", ]
  extra_row$admin_unit_name <- "Gilgit-Baltistan"
  extra_row$area_id <- config_row$AREA_ID; extra_row$fdef_id <- config_row$fdef_id; extra_row$ihme_id <- config_row$IHME_location_id; 
  extra_row$year <- 2014; 
  extra_row$api_low_pf <- extra_row$api_low_pv <- extra_row$api_high_pf <- extra_row$api_high_pv <- extra_row$api_mean_pf <- extra_row$api_mean_pv <- 0
  subnational_API_data <- rbind(subnational_API_data, extra_row)
}

if (!("Islamabad Capital Territory" %in% new_sub_list)){
  extra_row <- subnational_API_data[which(subnational_API_data$iso3 == "PAK")[1], ]
  config_row <- sub_config_file_2[sub_config_file_2$IHME_Location_ASCII_Name == "Islamabad Capital Territory", ]
  extra_row$admin_unit_name <- "Islamabad Capital Territory"
  extra_row$area_id <- config_row$AREA_ID; extra_row$fdef_id <- config_row$fdef_id; extra_row$ihme_id <- config_row$IHME_location_id; 
  extra_row$year <- 2014; 
  extra_row$api_low_pf <- extra_row$api_low_pv <- extra_row$api_high_pf <- extra_row$api_high_pv <- extra_row$api_mean_pf <- extra_row$api_mean_pv <- 0
  subnational_API_data <- rbind(subnational_API_data, extra_row)
...
subnational_list <- unique(data.frame("ISO3" = subnational_API_data$iso3, "country_name" = subnational_API_data$country_name, "Admin_Unit_Name" = subnational_API_data$admin_unit_name, "IHME_location_id" = subnational_API_data$ihme_id))
...
SMT_list <- data.frame("ISO3" = "IND", "country_name" = "India", "Admin_Unit_Name" = six_minor_terr, "IHME_location_id" = NA)
...
# Check structure:
ANC1 <- covar_list[[1]]
length(unique(ANC1$location_name)) # 1058.
# Note: GUF does not have any IHME covariates - borrow Suriname later.
# Check that covariates have new Pakistan units and up to 2019:
unique(ANC1$year_id) # Up to 2019.
ANC1[ANC1$location_name == "Islamabad Capital Territory", ]
...
# Read in treatment seeking data (updated for GBD2019):

national_data <- read.csv("J:/Treatment_Seeking/Data/FST_BySurvey_ALL.csv") # ADMIN0. 
national_data$CountryName <- as.character(national_data$CountryName)

# Can't do national name matching via ISO2 because e.g. Kyrgyzstan has DHS code as KY when ISO2 is KG.

national_data$CountryName[national_data$CountryName == "Congo Democratic Republic"] <- "Democratic Republic Of The Congo"
national_data$CountryName[national_data$CountryName == "Congo"] <- "Republic Of Congo"
national_data$CountryName[national_data$CountryName == "Kyrgyz Republic"] <- "Kyrgyzstan"
national_data$CountryName[national_data$CountryName == "Sao Tome and Principe"] <- "Sao Tome And Principe"
national_data$CountryName[national_data$CountryName == "Timor-Leste"] <- "East Timor"

str(national_data)
unique(national_data$CountryName)

subnational_data <- read.csv("J:/Treatment_Seeking/Data/subnational_TS_updated.csv")
subnational_data <- subnational_data[, 1:22]
head(subnational_data)

# Change Sao Paulo to avoid mismatch due to unicoding:
# subnational_data$Admin_Unit_Name <- as.character(subnational_data$Admin_Unit_Name)
# subnational_data$Admin_Unit_Name[subnational_data$Admin_Unit_Name == "Sao Paulo"] <- "São Paulo"
...
full_TreatSeek <- data.frame("ISO2" = NA, "ISO3" = NA, "GAUL_Code" = NA, "Country_Name" = NA, "Admin_Unit_Level" = NA, "Admin_Unit_Name" = NA, "WHO_Region" = NA, "WHO_Subregion" = NA, 
                                          ...
                                          "LDI_pc" = NA, "measles_vacc_cov_prop" = NA, "SBA_coverage_prop" = NA, "GDPpc_id_b2010" = NA, "prop_urban" = NA, "oop_hexp_cap" = NA, 
                                          "frac_oop_hexp" = NA, "measles_vacc_cov_prop_2" = NA, "ind_health" = NA, "education_all_ages_and_sexes_pc" = NA,
                                          "log_the_pc" = NA, "DMSP_nighttime" = NA, "accessibility" = NA, "VIIRS_nighttime" = NA)

# Fill in ADMIN0 data first:

for (i in 1:length(country_list)){ # Need to use new IHME populations for Cook islands, Monaco, Nauru, Niue, Palau, Saint Kitts and Nevis, San Marino, Tokelau, Tuvalu (i = 24, 64, 67, 72, 76, 84, 85, 104, 107) for now. 
  country_iso <- country_list[i]
  country_id <- config_file$IHME_location_id[config_file$ISO3 == country_iso]
  country_name <- config_file$MAP_Country_Name[config_file$ISO3 == country_iso]
  if (country_iso %in% c("GUF", "MYT")){
    if (country_iso == "GUF"){
  ...
  cov_matter <- rep(NA, length(cov_names))
  names(cov_matter) <- cov_names
  for (j in 1:length(years)){
    temp_middle_matter <- c(years[j], rep(NA, 7))
    if (sum(national_data$CountryName == as.character(country_name) & national_data$SurveyYear == years[j])>0){
      country_TS <- national_data[national_data$CountryName == as.character(country_name) & national_data$SurveyYear == years[j], ]
      # E.g. DOM has two surveys in 2013. This will be reflected in the dataframe used for fitting but not in this dataframe.
      temp_middle_matter <- country_TS[1, c("SurveyYear", "SurveyName", "fever_public_treat_wt.nationalprop", "fever_public_treat_wt.nationalprop_LCI_SVY", "fever_public_treat_wt.nationalprop_UCI_SVY", "fever_any_treat_wt.nationalprop", "fever_any_treat_wt.nationalprop_LCI_SVY", "fever_any_treat_wt.nationalprop_UCI_SVY")] 
    }
    if (country_iso == "TLS" & sum(subnational_data$Admin_Unit_Name == "East Timor" & subnational_data$SurveyYear == years[j], na.rm = TRUE)>0){
    ...
    for (k in 1:length(cov_names)){
      cov_values <- covar_list[[k]]
      if (length(unique(cov_values$sex_id)) > 1){
        print(paste("Covariate ", k, " is sex-stratified.", sep = ""))
        ...
        year_entry <- append(year_entry, cov_values$mean_value[cov_values$location_id == country_id & cov_values$year_id == years[j]])}
    }
    cov_matter <- rbind(cov_matter, year_entry[-1])
  }
  middle_matter <- middle_matter[-1, ]
  cov_matter <- cov_matter[-1, ]
  new_country_TS <- cbind(front_matter, middle_matter, cov_matter, end_matter, row.names = NULL)
...
for (i in 1:nrow(subnational_list)){ 
  ...
  country_name <- subnational_list$country_name[i]
  ...
  for (j in 1:length(years)){
    temp_middle_matter <- c(years[j], rep(NA, 7))
    year_entry <- NA
    if (sum(subnational_data$Admin_Unit_Name == as.character(unit_name) & subnational_data$CountryName == as.character(country_name) & subnational_data$SurveyYear == years[j], na.rm = TRUE)>0){
      unit_TS <- na.omit(subnational_data[subnational_data$Admin_Unit_Name == as.character(unit_name) & subnational_data$CountryName == as.character(country_name) & subnational_data$SurveyYear == years[j], ])
      ...
      if (nrow(temp_middle_matter)>1){ # Model average if have more than 1 row for a subnational unit from multiple DHS survey regions.
        avg_mean_1 <- mean(temp_middle_matter$fever_any_treat_wt.regionprop)
        avg_mean_2 <- mean(temp_middle_matter$fever_public_treat_wt.regionprop)
        avg_sd1 <- sqrt(sum((temp_middle_matter$fever_any_treat_wt.regionprop_UCI_SVY-temp_middle_matter$fever_any_treat_wt.regionprop_LCI_SVY)/(2*qnorm(0.975))^2))/nrow(temp_middle_matter)
        avg_sd2 <- sqrt(sum((temp_middle_matter$fever_public_treat_wt.regionprop_UCI_SVY-temp_middle_matter$fever_public_treat_wt.regionprop_LCI_SVY)/(2*qnorm(0.975))^2))/nrow(temp_middle_matter)
        temp_middle_matter <- temp_middle_matter[1, ]
        temp_middle_matter[, c("fever_any_treat_wt.regionprop", "fever_any_treat_wt.regionprop_LCI_SVY", "fever_any_treat_wt.regionprop_UCI_SVY")] <- rep(avg_mean_1, 3) + avg_sd1*qnorm(0.975)*c(0, -1, 1)
        temp_middle_matter[, c("fever_public_treat_wt.regionprop", "fever_public_treat_wt.regionprop_LCI_SVY", "fever_public_treat_wt.regionprop_UCI_SVY")] <- rep(avg_mean_2, 3) + avg_sd2*qnorm(0.975)*c(0, -1, 1)
      }
    }
    names(temp_middle_matter) <- c("Year", "SurveyName", "HMIS_treat", "HMIS_treat_low_SVY", "HMIS_treat_high_SVY", "Any_treat", "Any_treat_low_SVY", "Any_treat_high_SVY")
    middle_matter <- rbind(middle_matter, temp_middle_matter)
  ...
  cov_matter <- cov_matter[-1, ]
  middle_matter <- middle_matter[-1, ]
  new_unit_TS <- cbind(front_matter, middle_matter, cov_matter, end_matter, row.names = NULL)
  full_TreatSeek <- rbind(full_TreatSeek, new_unit_TS)
}

i # Should be 384 if OK.
...
boxcox(full_TreatSeek$ANC1_coverage_prop ~ 1) # Use log/transformation that corresponds to the nearest lambda integer.
...
full_TreatSeek_n[, 23:41] <- apply(full_TreatSeek_n[, 23:41], MARGIN = 2, FUN = function(x){(x - mean(x))/sd(x)}) 
...
# Set Western Europe to South East Asia since High-income Asia Pacific is with Southeast Asia.
full_TreatSeek_n$IHME_Region_Name[full_TreatSeek_n$IHME_Region_Name == 'Western Europe'] <- 'Southeast Asia'
...
Any_outliers <- which((clean_TreatSeek_Any$ISO3 == "IND" & clean_TreatSeek_Any$Year == 1993) | (clean_TreatSeek_Any$ISO3 == "PAK" & clean_TreatSeek_Any$Year == 1991) | (clean_TreatSeek_Any$ISO3 == "NGA" & clean_TreatSeek_Any$Year == 2008) | (clean_TreatSeek_Any$ISO3 == "BRA" & clean_TreatSeek_Any$Year == 1996))
HMIS_outliers <- which((clean_TreatSeek_HMISfrac$ISO3 == "IND" & clean_TreatSeek_HMISfrac$Year == 1993) | (clean_TreatSeek_HMISfrac$ISO3 == "PAK" & clean_TreatSeek_HMISfrac$Year == 1991) | (clean_TreatSeek_HMISfrac$ISO3 == "NGA" & clean_TreatSeek_HMISfrac$Year == 2008) | (clean_TreatSeek_HMISfrac$ISO3 == "BRA" & clean_TreatSeek_HMISfrac$Year == 1996))
...
length(train.id)/nrow(clean_TreatSeek_Any) # 0.7176148.
...
length(train.id)/nrow(clean_TreatSeek_HMISfrac) #  0.7172272.
...
for (TS_i in 1:N_TS){
  ...
  for (i in 1:length(country_list)){
    ...
    for (j in 1:length(years)){
      ...
      if (sum(national_data$CountryName == as.character(country_name) & national_data$SurveyYear == years[j])>0){
        ...
        if (country_TS[1, c("fever_public_treat_wt.nationalprop_UCI_SVY")] !=  country_TS[1, c("fever_public_treat_wt.nationalprop_LCI_SVY")] & !is.na(country_TS[1, c("fever_public_treat_wt.nationalprop_LCI_SVY")])){
          temp_middle_matter[3] <- rnorm(1, mean = country_TS[1, c("fever_public_treat_wt.nationalprop")], sd = (country_TS[1, c("fever_public_treat_wt.nationalprop_UCI_SVY")] - country_TS[1, c("fever_public_treat_wt.nationalprop_LCI_SVY")])/(2*qnorm(0.975)))
        }else{temp_middle_matter[3] <- country_TS[1, c("fever_public_treat_wt.nationalprop")]}
        if (country_TS[1, c("fever_any_treat_wt.nationalprop_UCI_SVY")] !=  country_TS[1, c("fever_any_treat_wt.nationalprop_LCI_SVY")] & !is.na(country_TS[1, c("fever_any_treat_wt.nationalprop")])){
          temp_middle_matter[4] <- rnorm(1, mean = country_TS[1, c("fever_any_treat_wt.nationalprop")], sd = (country_TS[1, c("fever_any_treat_wt.nationalprop_UCI_SVY")] - country_TS[1, c("fever_any_treat_wt.nationalprop_LCI_SVY")])/(2*qnorm(0.975)))
        }else{temp_middle_matter[4] <- country_TS[1, c("fever_any_treat_wt.nationalprop")]}
      }
      if (country_iso == "TLS" & sum(subnational_data$Admin_Unit_Name == "East Timor" & subnational_data$SurveyYear == years[j], na.rm = TRUE)>0){
          country_TS <- na.omit(subnational_data[subnational_data$Admin_Unit_Name == "East Timor" & subnational_data$SurveyYear == years[j], ])
          temp_middle_matter[2] <- as.character(country_TS[1, c("SurveyName")])
  ...
  for (i in 1:nrow(subnational_list)){
    ...
    for (j in 1:length(years)){
      ...
      if (sum(subnational_data$Admin_Unit_Name == as.character(unit_name) & subnational_data$SurveyYear == years[j], na.rm = TRUE)>0){
        unit_TS <- na.omit(subnational_data[subnational_data$Admin_Unit_Name == as.character(unit_name) & subnational_data$CountryName == as.character(country_name) & subnational_data$SurveyYear == years[j], ])
        temp_middle_matter[2] <- unit_TS[1 , c("SurveyName")]
        if (nrow(unit_TS)>1){ # Model average if have more than 1 row for a subnational unit from multiple DHS survey regions.
          avg_mean_1 <- mean(unit_TS$fever_any_treat_wt.regionprop)
          avg_mean_2 <- mean(unit_TS$fever_public_treat_wt.regionprop)
          avg_sd1 <- sqrt(sum((unit_TS$fever_any_treat_wt.regionprop_UCI_SVY-unit_TS$fever_any_treat_wt.regionprop_LCI_SVY)/(2*qnorm(0.975))^2))/nrow(unit_TS)
          avg_sd2 <- sqrt(sum((unit_TS$fever_public_treat_wt.regionprop_UCI_SVY-unit_TS$fever_public_treat_wt.regionprop_LCI_SVY)/(2*qnorm(0.975))^2))/nrow(unit_TS)
          temp_middle_matter[3] <- rnorm(1, mean = avg_mean_2, sd = avg_sd2)
          temp_middle_matter[4] <- rnorm(1, mean = avg_mean_1, sd = avg_sd1)
  ...
  full_TreatSeek_i[, 12:30] <- apply(full_TreatSeek_i[, 12:30], MARGIN = 2, FUN = function(x){(x - mean(x))/sd(x)}) 
  ...
  # Set High-income Asia to South East Asia since it is the closest geographically; set Southern Latin America to Tropical Latin America because it has higher income per capita compared to Andean Latin America. 
  # Set Western Europe to South East Asia since High-income Asia is with South East Asia.
  
  full_TreatSeek_i$IHME_Region_Name[full_TreatSeek_i$IHME_Region_Name == 'Western Europe'] <- 'Southeast Asia'
...
for (k in 1:N_cov){
  ...
  for (i in 1:length(country_list)){
    ...
    country_id <- config_file$IHME_location_id[config_file$ISO3 == country_iso]
    if (country_iso == "GUF"){
        country_id <- config_file$IHME_location_id[config_file$ISO3 == "SUR"] 
    }
    if (country_iso == "MYT"){
        country_id <- config_file$IHME_location_id[config_file$ISO3 == "COM"] # MYT uses Comoros IHME covariates for now.
    }
    ...
    new_country_TS <- cbind(front_matter, end_matter, row.names = NULL)
  ...
  # Set High-income Asia to South East Asia since it is the closest geographically; set Southern Latin America to Tropical Latin America because it has higher income per capita compared to Andean Latin America. 
  covariate_i$IHME_Region_Name[covariate_i$IHME_Region_Name == 'Western Europe'] <- 'Southeast Asia'
  ...
  pdf(paste(graphics.path, names(covar_list)[k], '.pdf', sep = ''),width=8.7,height = 11.2)
  ...
  for (l in 1:length(master.region.list)){
    ...
    for (m in 1:length(countries)){
      ...
      for (n in 1:nrow(unit.list)){
        ...
        plot(0,0,type='n',c(min(covariate_i$cov_low, na.rm = TRUE),max(covariate_i$cov_high, na.rm = TRUE)),xlim=c(years[1], years[length(years)]),main=paste(master.region.list[l], ': ', unit.list$Admin_Unit_Name[n] , ", ", countries[m], sep = ''), ylab='IHME covariate', xlab='Year')
