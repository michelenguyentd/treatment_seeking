...
for (i in 1:length(cov_ind)){
  ...
  gam_Any <- gamm(logit_Any ~ s(Year, by = Time_Factor) + s(X), data = data.frame("logit_Any" = train_data_Any$logit_Any, "X" = var_values_Any, 'Year' = train_data_Any$Year, 'Time_Factor' = train_data_Any$Time_Factor, 'IHME_Region_Name' = train_data_Any$IHME_Region_Name, 'Admin_Unit_Name' = train_data_Any$Admin_Unit_Name), random = list(IHME_Region_Name = ~ 1, Admin_Unit_Name = ~ 1))
  AIC_Any[i] <- AIC(gam_Any$lme)
  var_values_HMIS <- train_data_HMISfrac[, cov_ind[i]]
  gam_HMIS <- gamm(logit_HMIS ~ s(Year, by = Time_Factor_2) + s(X), data =  data.frame("logit_HMIS" = train_data_HMISfrac$logit_HMIS, "X" = var_values_HMIS, 'Year' = train_data_HMISfrac$Year, 'Time_Factor_2' = train_data_HMISfrac$Time_Factor_2, 'IHME_Region_Name' = train_data_HMISfrac$IHME_Region_Name, 'Admin_Unit_Name' = train_data_HMISfrac$Admin_Unit_Name), random = list(IHME_Region_Name = ~ 1, Admin_Unit_Name = ~ 1))
...
formula_any <-  logit_Any ~ s(Year, by = Time_Factor, k = 5) + s(ANC1_coverage_prop, k = 3) + s(log_the_pc, k = 3) + s(accessibility, k = 3) + s(VIIRS_nighttime, k = 3) + s(hospital_beds_per1000, k = 3) + s(measles_vacc_cov_prop_2, k = 3) + s(IFD_coverage_prop, k = 3) + s(prop_urban, k = 3)
test.model <- gamm(formula_any, data = train_data_Any, random = list(IHME_Region_Name = ~ 1, Admin_Unit_Name = ~ 1))

summary(test.model$gam)

model_any <- uGamm(logit_Any ~ s(Year), data = train_data_Any, random = list(IHME_Region_Name = ~ 1, Admin_Unit_Name = ~ 1))
...
formula_HMIS <-  logit_HMIS ~ s(Year, by = Time_Factor_2, k = 5)  + s(VIIRS_nighttime, k = 3) + s(accessibility, k = 3) + s(hospital_beds_per1000) + s(IFD_coverage_prop, k = 3) + s(oop_hexp_cap, k = 3) + s(measles_vacc_cov_prop_2, k = 3) + s(education_all_ages_and_sexes_pc, k = 3) + s(ANC1_coverage_prop, k = 3) + s(ind_health, k = 3)

test.model <- gamm(formula_HMIS, data = train_data_HMISfrac, random = list(IHME_Region_Name = ~ 1, Admin_Unit_Name = ~ 1))

summary(test.model$gam)

model_HMIS <- uGamm(logit_HMIS ~ s(Year), data = train_data_HMISfrac, random = list(IHME_Region_Name = ~ 1, Admin_Unit_Name = ~ 1))
...
formula_any_1 <-  logit_Any ~ s(Year, by = Time_Factor, k = 5) + s(ANC1_coverage_prop, k = 3) + s(log_the_pc, k = 3) + s(VIIRS_nighttime, k = 3) 

model_any_1 <- gamm(formula_any_1, data = train_data_Any, random = list(IHME_Region_Name = ~ 1, Admin_Unit_Name = ~ 1))
...
qqnorm(refRegion$Admin_Unit_Name[, ], main = 'Normal Q-Q plot (Unit)')
...
model_hmis_1 <- gamm(formula_hmis_1, data = train_data_HMISfrac, random = list(IHME_Region_Name = ~ 1, Admin_Unit_Name = ~ 1))
...
qqnorm(refRegion_HMIS$Admin_Unit_Name[, ], main = 'Normal Q-Q plot (Unit)')
