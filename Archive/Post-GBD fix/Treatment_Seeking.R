...
#library(doMC) # Depends on the mclapply function, not available for Windows.
#registerDoMC(cores=10)

# library(foreach)
# library(doParallel)
# workers <- makeCluster(8) # I have 8 cores in my computer.
...
nrow(admin0_units)
# config_file_circ <- read.csv("Z:/GBD2017/Processing/Stages/02b_Model_TS/Archive/Verified_Outputs/treat_seek_any_no_year_19_Jul_2017.csv")
# length(unique(config_file_circ$iso))
# unique(config_file_circ$iso[!(config_file_circ$iso %in% admin0_units$ISO3)]) # United Arab Emirates, Mayotte.

# Create list of iso for countries to model:
gdb_2016_country_list <- admin0_units$ISO3

# Vector of numeric Year to model:
master_year <- 1980:2017 # For GBD2017, 1980:2016.
...
# Check to see if Indicator Year used != Year:

sum(TreatSeek$Electricity_IndicatorYearUsed != TreatSeek$Year) # 10 years. 
sum(TreatSeek$GDP_IndicatorYearUsed != TreatSeek$Year) # All same.
sum(TreatSeek$GDPGrowth_IndicatorYearUsed != TreatSeek$Year) # All same.
sum(TreatSeek$GDPpercap_IndicatorYearUsed != TreatSeek$Year) # All same.
sum(TreatSeek$GNIpercap_IndicatorYearUsed != TreatSeek$Year) # 1 year.
sum(TreatSeek$HealthExpercap_IndicatorYearUsed != TreatSeek$Year) # 11 years.
sum(TreatSeek$HealthExpPub_IndicatorYearUsed != TreatSeek$Year) # All same.
sum(TreatSeek$HealthExTotal_IndicatorYearUsed != TreatSeek$Year) # 11 years.
sum(TreatSeek$Literacy_IndicatorYearUsed != TreatSeek$Year) # 122 years.
sum(TreatSeek$PregWomenCare_IndicatorYearUsed != TreatSeek$Year) # 41 years.
sum(TreatSeek$RuralPop_IndicatorYearUsed != TreatSeek$Year) # All same.
sum(TreatSeek$UrbanPop_IndicatorYearUsed != TreatSeek$Year) # All same.
sum(TreatSeek$CommHlthWrkrs_IndicatorYearUsed != TreatSeek$Year) # There exists NAs.
sum(TreatSeek$DPT_IndicatorYearUsed != TreatSeek$Year) # 2 years. 
sum(TreatSeek$NurseMidwives_IndicatorYearUsed != TreatSeek$Year) # 135 years.

# Remove WB covariates in Treatment seeking data to be replaced with gap-filled versions:

TreatSeek <- TreatSeek[, 1:33]

# Add columns for WB covariates which were eventually used in the models (note we don't have the model selection process):

file_name<-c('Z:/GBD2017/Processing/Stages/01b2_World_Bank_Covariates/Checkpoint_Outputs/public_exp_jul_2017.csv',
             'Z:/GBD2017/Processing/Stages/01b2_World_Bank_Covariates/Checkpoint_Outputs/pregnant_jul_2017.csv', 
             'Z:/GBD2017/Processing/Stages/01b2_World_Bank_Covariates/Checkpoint_Outputs/out_of_pocket.csv', 
             'Z:/GBD2017/Processing/Stages/01b2_World_Bank_Covariates/Checkpoint_Outputs/primary_school_jul_2017.csv')

for (i in 1:length(file_name)){
  temp_col <- rep(NA, nrow(TreatSeek))
  current_cov <- read.csv(file_name[i],sep=';',stringsAsFactors=F)
  for (i in 1:nrow(TreatSeek)){
    row_num<-which(as.character(current_cov$iso) == as.character(TreatSeek$ISO3[i]) & current_cov$year == TreatSeek$Year[i])
    temp_col[i]<-current_cov$gp_value[row_num]
  }
  TreatSeek <- cbind(TreatSeek, temp_col)
}

names(TreatSeek) <- c(names(TreatSeek)[1:33], "HealthExPub", "PregWomenCare", "Out_pocket", "PrimaryComplete")
...
subnational = FALSE # If predicting for India subnationals only. Repeat for subnational = FALSE for national estimates.
...
for (i_sim in 1 :10){ 
  ...
  if (subnational == TRUE){
    ...
    sub_matrix$Year <- factor( sub_matrix $Year,levels = master_year) 
    ...
    cleandata_any$Year<-factor(cleandata_any$Year,levels = master_year) # Need to be updated every year; so as to get d_m3 matrix.
  ...
  file_name<-c('Z:/GBD2017/Processing/Stages/01b2_World_Bank_Covariates/Checkpoint_Outputs/primary_school_jul_2017.csv',
    'Z:/GBD2017/Processing/Stages/01b2_World_Bank_Covariates/Checkpoint_Outputs/pregnant_jul_2017.csv')
  ...
  for (i_pred in 1:10){ # Previously parallel but hit problems with proper 'qr' component for lm object. 
    ...
    for (cnty in rownames(d_m3)){
      ...
      year_data<-length(master_year)-length(which(is.na(seek_value))) # Number of years with data.
      ...
      {  
        ...
        if (value<length(master_year)) # value is not the last data point. 
...
pred_algo_data<-data.frame(iso=rep(rownames(d_m3),each=length(master_year)),year= rep(master_year,length(rownames(d_m3))))
...
if (subnational == TRUE){
  ...
  cleandata <- droplevels(TreatSeek[,c('Any_treat','WHO_Subregion','Year','PregWomenCare','PrimaryComplete','Country_Name','ISO3','Any_treat_low_SVY', 'Any_treat_high_SVY','SurveyName')])
...
cleandata$point_color[grep('MIS',cleandata$SurveyName)]<-'blue'
...
for( i in unique(treat_seek_any$iso) ){
  ...
  plot(0,0,type='n',ylim=c(0,1),xlim=c(master_year[1], master_year[length(master_year)]),main=i,ylab='% Sought Treatment',xlab='Year')
  
  plotCI(master_year,country_db$mean,ui=country_db$upper,li=country_db$lower,add=T)
...
for (i_sim in 1:10){ 
    ...
    if (subnational == TRUE){
      ...
      cleandata_hmis$Year<-factor(cleandata_hmis$Year,levels = master_year) # Need to be updated every year; so as to get d_m3 matrix.
  ...
  if (subnational == TRUE){
    ...
    cleandata <- droplevels(TreatSeek[,c('HMIS_treat','WHO_Subregion','Year','HealthExPub','PregWomenCare','PrimaryComplete', 'Country_Name','ISO3','HMIS_treat_low_SVY', 'HMIS_treat_high_SVY','SurveyName')])
  ...
  for( i in unique(treat_seek_pub2$iso) ){
    ...
    plot(0,0,type='n',ylim=c(0,1),xlim=c(master_year[1], master_year[length(master_year)]),main=i,ylab='% Sought Public Treatment',xlab='Year')
