...
# Restrict analysis to EW Africa or SE Asia:

model_no <- "SEAsia_" # 1. "EWAfrica_"; 2. "SEAsia_"

# train_data_Any <- train_data_Any[train_data_Any$IHME_Region_Name %in% c("Eastern Sub-Saharan Africa", "Western Sub-Saharan Africa"), ]
# train_data_HMISfrac <-  train_data_HMISfrac[train_data_HMISfrac$IHME_Region_Name %in% c("Eastern Sub-Saharan Africa", "Western Sub-Saharan Africa"), ]
# test_data_Any <-  test_data_Any[test_data_Any$IHME_Region_Name %in% c("Eastern Sub-Saharan Africa", "Western Sub-Saharan Africa"), ]
# test_data_HMISfrac <-  test_data_HMISfrac[test_data_HMISfrac$IHME_Region_Name %in% c("Eastern Sub-Saharan Africa", "Western Sub-Saharan Africa"), ]

train_data_Any <- train_data_Any[train_data_Any$IHME_Region_Name == 'Southeast Asia', ]
train_data_HMISfrac <-  train_data_HMISfrac[train_data_HMISfrac$IHME_Region_Name == 'Southeast Asia', ]
test_data_Any <-  test_data_Any[test_data_Any$IHME_Region_Name == 'Southeast Asia', ]
test_data_HMISfrac <-  test_data_HMISfrac[test_data_HMISfrac$IHME_Region_Name == 'Southeast Asia', ]
...
for (i in 1:length(cov_ind)){
  ...
  gam_Any <- gam(logit_Any ~ s(Year) + s(X), data = data.frame("logit_Any" = train_data_Any$logit_Any, "X" = var_values_Any, 'Year' = train_data_Any$Year, 'IHME_Region_Name' = train_data_Any$IHME_Region_Name))
  var_values_HMIS <- train_data_HMISfrac[, cov_ind[i]]
  gam_HMIS <- gam(logit_HMIS ~ s(Year) + s(X), data =  data.frame("logit_HMIS" = train_data_HMISfrac$logit_HMIS, "X" = var_values_HMIS, 'Year' = train_data_HMISfrac$Year, 'IHME_Region_Name' = train_data_HMISfrac$IHME_Region_Name))
  AIC_Any[i] <- gam_Any$aic
  AIC_HMIS[i] <- gam_HMIS$aic
}

AIC_Any_order <- names(sort(AIC_Any[!(names(AIC_Any) %in% c("frac_oop_hexp", "ind_health"))])) # Don't use frac_oop_hexp for any TS. Exclude ind_health too - strange relation with public fractions.
...
AIC_HMIS_order <- names(sort(AIC_HMIS[names(AIC_HMIS) != "ind_health"]))
...
#  Use covariates identified in test_cov_Any for EW Africa:

formula_any <-  logit_Any ~ s(Year) + s(ANC1_coverage_prop, k = 3) + s(VIIRS_nighttime, k = 3) + s(measles_vacc_cov_prop_2, k = 3) + s(GDPpc_id_b2010, k = 3) + s(accessibility, k = 3) + s(IFD_coverage_prop, k = 3) +  s(hospital_beds_per1000, k = 3) 
test.model <- gam(formula_any, data = train_data_Any)

summary(test.model)

model_any <- uGamm(logit_Any ~  s(Year), data = train_data_Any)

model_any$gam$formula <- formula_any

temptime <- proc.time()[3]
model.select.any <- dredge(model_any, fixed=c("s(Year)"), m.min=4)
timetaken <- proc.time()[3] - temptime  # 15 min.
...
#  Use covariates identified in test_cov_Any for SE Asia:

formula_HMIS <-  logit_HMIS ~ s(Year) + s(accessibility, k = 3) + s(ANC4_coverage_prop, k = 3) + s(VIIRS_nighttime, k = 3) +  s(measles_vacc_cov_prop, k = 3) + s(oop_hexp_cap, k = 3) + s(IFD_coverage_prop, k = 3) + s(frac_oop_hexp, k = 3) + s(measles_vacc_cov_prop_2, k = 3) + s(hospital_beds_per1000, k = 3)

test.model <- gam(formula_HMIS, data = train_data_HMISfrac)

summary(test.model)

model_HMIS <- uGamm(logit_HMIS ~ s(Year), data = train_data_HMISfrac)

model_HMIS$gam$formula <- formula_HMIS

temptime2 <- proc.time()[3]
model.select.HMIS <- dredge(model_HMIS, fixed=c('s(Year)'), m.min=4)
...
formula_any_1 <-  logit_Any ~ s(Year) + s(ANC1_coverage_prop, k = 3) + s(GDPpc_id_b2010, k = 3) + s(IFD_coverage_prop, k = 3)
# formula_any_2 <-  logit_Any ~ s(Year, k = 7) + s(ANC4_coverage_prop, k = 3) + s(DMSP_nighttime, k = 3)
...
# gamtabs(model_any_1, caption = "Summary of best any TS model with IHME region temporal trends and factors")

pdf(paste(graphics.path, model_no, "Any_model_smooths_1.pdf", sep = ""), width = 8, height = 8)
par(mfrow = c(2, 2))
...
# pdf(paste(graphics.path, model_no, "Any_model_smooths_2.pdf", sep = ""), width = 12, height = 4)
# par(mfrow = c(1, 3))
# plot(model_any_2, select = 1, ylim = c(-5, 5), xlim = c(1980, 2017))
# plot(model_any_2, select = 2, ylim = c(-5, 5))
# plot(model_any_2, select = 3, ylim = c(-5, 5))
# dev.off()

formula_hmis_1 <-  logit_HMIS ~ s(Year) + s(ANC1_coverage_prop, k = 3) + s(measles_vacc_cov_prop, k = 3) + s(oop_hexp_cap, k = 3) + s(VIIRS_nighttime, k = 3)

model_hmis_1 <- gam(formula_hmis_1, data = train_data_HMISfrac)
summary(model_hmis_1) 

# formula_hmis_2 <-  logit_HMIS ~ s(Year, k = 3) + s(ANC1_coverage_prop, k = 3) + s(DMSP_nighttime, k = 3) + s(IFD_coverage_prop, k = 3)
# 
# model_hmis_2 <- gam(formula_hmis_2, data = train_data_HMISfrac)
# summary(model_hmis_2) 


# # To be changed for different models:
# 
# gamtabs(model_hmis_1, caption = "Summary of best public fraction model with IHME region temporal trends and factors")

pdf(paste(graphics.path, model_no, "HMIS_model1_smooths.pdf", sep = ""), width = 8, height = 8)
par(mfrow = c(2, 2))
plot(model_hmis_1, select = 1, ylim = c(-5, 5)) 
plot(model_hmis_1, select = 2, ylim = c(-5, 5))  
plot(model_hmis_1, select = 3, ylim = c(-5, 5)) 
plot(model_hmis_1, select = 4, ylim = c(-5, 5)) 
dev.off()

pdf(paste(graphics.path, model_no, "HMIS_model2_smooths.pdf", sep = ""), width = 8, height = 8)
par(mfrow = c(2, 2))
plot(model_hmis_2, select = 1, ylim = c(-5, 5), xlim = c(1980, 2017)) 
plot(model_hmis_2, select = 2, ylim = c(-5, 5))  
plot(model_hmis_2, select = 3, ylim = c(-5, 5)) 
plot(model_hmis_2, select = 4, ylim = c(-5, 5)) 
dev.off()
