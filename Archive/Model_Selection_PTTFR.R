...
formula_any <-  logit_Any ~ -1 + Reg_Factor + s(Year, by = Time_Factor, k = 5) + s(ANC1_coverage_prop, k = 3) + s(log_the_pc, k = 3) + s(measles_vacc_cov_prop_2, k = 3) + s(DMSP_nighttime, k = 3) + s(prop_urban, k = 3) + s(hospital_beds_per1000, k = 3) + s(IFD_coverage_prop, k = 3) 
...
formula_HMIS <-  logit_HMIS ~ -1 + Reg_Factor_2 + s(Year, by = Time_Factor_2, k = 5)  + s(VIIRS_nighttime, k = 3) +  s(accessibility, k = 3) + s(oop_hexp_cap, k =3) + s(IFD_coverage_prop, k = 3) + s(measles_vacc_cov_prop_2, k = 3) + s(ANC1_coverage_prop, k = 3) + s(education_all_ages_and_sexes_pc, k = 3) + s(hospital_beds_per1000, k = 3) + s(ind_health, k = 3) 
...
formula_any_1 <-  logit_Any ~ -1 + Reg_Factor + s(Year, by = Time_Factor, k = 5) + s(ANC1_coverage_prop, k = 3) + s(log_the_pc, k = 3) + s(DMSP_nighttime, k = 3)

model_any_1 <- gam(formula_any_1, data = train_data_Any)
summary(model_any_1) # PTTIR: 81%
...
formula_hmis_1 <-  logit_HMIS ~ -1 + Reg_Factor_2 + s(Year, by = Time_Factor_2, k = 5) + s(accessibility, k = 3) + s(hospital_beds_per1000, k = 3) + s(IFD_coverage_prop, k = 3) + s(education_all_ages_and_sexes_pc, k = 3)

model_hmis_1 <- gam(formula_hmis_1, data = train_data_HMISfrac)
summary(model_hmis_1) # 65.1%.
...
# 
# train.error.any <- sqrt(mean(Any_model$residuals^2)) # 0.6294914. (logit scale)
# train.error.hmis <- sqrt(mean((train.predict.HMIS - train_data_HMISfrac$logit_HMIS)^2)) #0.6244664. (logit scale)
# 
# test.error.any <- sqrt(mean((test.predict.any - test_data_Any$logit_Any)^2)) #0.6510353. (logit scale)
# test.error.hmis <- sqrt(mean((test.predict.HMIS - test_data_HMISfrac$logit_HMIS)^2)) # 0.7809577. (logit scale)
# 
# train.error.any.raw <- sqrt(mean((train_data_Any$Any_treat - inv.logit(Any_model$fitted.values))^2)) #0.1165209.
# train.error.hmis.raw <- sqrt(mean((train_data_HMISfrac$HMIS_frac - inv.logit(train.predict.HMIS))^2)) #0.1250249.
# 
# test.error.any.raw <- sqrt(mean((test_data_Any$Any_treat - inv.logit(test.predict.any))^2)) #0.1262028.
