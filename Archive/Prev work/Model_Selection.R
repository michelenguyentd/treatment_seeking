...
train_data_Any$Time_Factor[train_data_Any$IHME_Region_Name == 'South Asia'] <- 'South Asia'
...
test_data_Any$Time_Factor[test_data_Any$IHME_Region_Name == 'South Asia'] <- 'South Asia'
...
train_data_HMISfrac$Time_Factor[train_data_HMISfrac$IHME_Region_Name == 'South Asia'] <- 'South Asia'
...
test_data_HMISfrac$Time_Factor[test_data_HMISfrac$IHME_Region_Name == 'South Asia'] <- 'South Asia'
...
formula_any <-  logit_Any ~ -1 + IHME_Region_Name + SMT_Factor + s(Year, by = Time_Factor, k = 7) + s(DMSP_nighttime, k = 3) + s(ANC4_coverage_prop, k = 3) + s(log_the_pc, k = 3) + s(accessibility, k = 3) + s(measles_vacc_cov_prop, k = 3) + s(hospital_beds_per1000, k = 3) + s(measles_vacc_cov_prop_2, k = 3)
...
formula_HMIS <-  logit_HMIS ~ -1 + IHME_Region_Name + SMT_Factor + s(Year, by = Time_Factor) +  s(frac_oop_hexp, k = 3) + s(DTP3_coverage_prop, k = 3) + s(education_all_ages_and_sexes_pc, k = 3) + s(hospital_beds_per1000, k = 3) + s(accessibility, k = 3) + s(VIIRS_nighttime, k = 3) + s(oop_hexp_cap, k = 3) + s(measles_vacc_cov_prop_2, k = 3) + s(ind_health, k = 3)    
...
formula_any_1 <-  logit_Any ~ -1 + IHME_Region_Name + SMT_Factor + s(Year, by = Time_Factor, k = 7) + s(ANC4_coverage_prop, k = 3) + s(DMSP_nighttime, k = 3) + s(log_the_pc, k = 3)
formula_any_2 <-  logit_Any ~ -1 + IHME_Region_Name + SMT_Factor + s(Year, by = Time_Factor, k = 7) + s(ANC4_coverage_prop, k = 3) + s(DMSP_nighttime, k = 3)

model_any_1 <- gam(formula_any_1, data = train_data_Any)
summary(model_any_1) # 71.9%
...
formula_hmis_1 <-  logit_HMIS ~ -1 + IHME_Region_Name + SMT_Factor + s(Year, by = Time_Factor) + s(frac_oop_hexp, k = 3) + s(education_all_ages_and_sexes_pc, k = 3)+ s(accessibility, k = 3) + s(hospital_beds_per1000, k = 3) 
...
summary(model_hmis_1) # 69.5% deviance explained.
...
train.error.any <- sqrt(mean(Any_model$residuals^2)) # 0.5542392. (logit scale)
train.error.hmis <- sqrt(mean((train.predict.HMIS - train_data_HMISfrac$logit_HMIS)^2)) #0.7059329. (logit scale)

test.error.any <- sqrt(mean((test.predict.any - test_data_Any$logit_Any)^2)) #0.6589585. (logit scale)
test.error.hmis <- sqrt(mean((test.predict.HMIS - test_data_HMISfrac$logit_HMIS)^2)) # 0.6517518. (logit scale)

train.error.any.raw <- sqrt(mean((train_data_Any$Any_treat - logit(Any_model$fitted.values, inverse = TRUE))^2)) #0.1131629.
train.error.hmis.raw <- sqrt(mean((train_data_HMISfrac$HMIS_frac - logit(train.predict.HMIS, inverse = TRUE))^2)) #0.1381154. 

test.error.any.raw <- sqrt(mean((test_data_Any$Any_treat - logit(test.predict.any, inverse = TRUE))^2)) #0.1229238. 
test.error.hmis.raw <- sqrt(mean((test_data_HMISfrac$HMIS_frac - logit(test.predict.HMIS, inverse = TRUE))^2)) #0.1388919. 
