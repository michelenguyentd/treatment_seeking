...
# API_mean <- country_sub.api.1; API_low <- country_sub.api.low.1; API_high <- country_sub.api.high.1; covariates <- covariates.1

gapfill_API_timeseries_rt <- function(API_mean,API_low,API_high,covariates) {
  ...
  compile("gapfill_API_timeseries_rt.cpp") # Compile the C++ file.
  dyn.load(dynlib("gapfill_API_timeseries_rt")) # Dynamically link the C++ code.
  ...
  obj <- MakeADFun(data,parameters,random=c('national_log_initial_API_offsets','national_shortterm_noise','national_longterm_noise'),DLL="gapfill_API_timeseries_rt")
  ...
  #compile("Code/gapfill_API_timeseries_fixedpar_pf.cpp")
  
  compile("gapfill_API_timeseries_fixedpar_rt.cpp") # Compile the C++ file.
  dyn.load(dynlib("gapfill_API_timeseries_fixedpar_rt")) 
  
  obj2 <- MakeADFun(data,parameters,random=c('national_log_initial_API_offsets','national_shortterm_noise','national_longterm_noise'),DLL="gapfill_API_timeseries_fixedpar_rt")
  ...
  for (i in 1:1000) {
    buffer <- exp(obj2$report(r.draws[i,])$log_API_timeseries) # Computes API_timeseries matrix based on the simulations and model (Monte Carlo, prior predictive distribution).
