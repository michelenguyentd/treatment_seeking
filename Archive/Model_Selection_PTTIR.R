...
formula_any <-  logit_Any ~ -1 + IHME_Region_Name + s(Year, by = Time_Factor, k = 5) + s(ANC1_coverage_prop, k = 3) + s(log_the_pc, k = 3) + s(measles_vacc_cov_prop_2, k = 3) + s(DMSP_nighttime, k = 3) + s(prop_urban, k = 3) + s(hospital_beds_per1000, k = 3) + s(IFD_coverage_prop, k = 3)
...
timetaken <- proc.time()[3] - temptime  # About 2 minutes.
model.select.any[1:10]

formula_HMIS <-  logit_HMIS ~ -1 + IHME_Region_Name + s(Year, by = Time_Factor_2, k = 5) + s(VIIRS_nighttime, k = 3) + s(accessibility, k = 3) + s(IFD_coverage_prop, k = 3) + s(oop_hexp_cap, k = 3) + s(measles_vacc_cov_prop_2, k = 3) + s(ANC1_coverage_prop, k = 3)  + s(education_all_ages_and_sexes_pc, k = 3) + s(hospital_beds_per1000, k = 3) + s(ind_health, k = 3)
...
best.HMIS<-subset(model.select.HMIS, delta < 20) #takes the best model with delta AICc less than 2 - only 1. 
...
formula_any_1 <-  logit_Any ~ -1 + IHME_Region_Name + s(Year, by = Time_Factor, k = 5) + s(ANC1_coverage_prop, k = 3) + s(DMSP_nighttime, k = 3) + s(IFD_coverage_prop, k = 3) + s(log_the_pc, k =3)

model_any_1 <- gam(formula_any_1, data = train_data_Any)
sum_any_1 <- summary(model_any_1) # PTTIR: 79.6%

reg_coeff <- sum_any_1$p.coeff
col.id <- rep(1, length(reg_coeff))
col.id[names(reg_coeff) %in% c('IHME_Region_NameEastern Sub-Saharan Africa', 'IHME_Region_NameSoutheast Asia', 'IHME_Region_NameSouth Asia')] <- 2
...
abline(h = 0, lty = 2)
abline(h = mean(reg_coeff[!(names(reg_coeff) %in% c('IHME_Region_NameEastern Sub-Saharan Africa', 'IHME_Region_NameSoutheast Asia', 'IHME_Region_NameSouth Asia'))]), col = 2)
axis(side=1, at=1:length(reg_coeff), labels = c("ALA", 'Car.', 'C.Asia', 'CLA', 'CSA', 'ESA', 'NAME', 'S.Asia', 'SE.Asia', 'SSA', 'TLA', 'WSA'))
legend('topright', lty = 1, col = 2, legend = c('Mean of non-significantly 0'))
dev.off()
...
formula_hmis_1 <-  logit_HMIS ~ -1 + IHME_Region_Name + s(Year, by = Time_Factor_2, k = 5) + s(accessibility, k = 3) + s(education_all_ages_and_sexes_pc, k = 3) + s(hospital_beds_per1000, k = 3) + s(IFD_coverage_prop, k = 3)

model_hmis_1 <- gam(formula_hmis_1, data = train_data_HMISfrac)
sum_hmis_1 <- summary(model_hmis_1) # 66%.
...
col.id_2[names(reg_coeff_2) %in% c('IHME_Region_NameAndean Latin America', 'IHME_Region_NameCentral Asia', 'IHME_Region_NameSoutheast Asia', 'IHME_Region_NameEastern Sub-Saharan Africa', 'IHME_Region_NameSouth Asia', 'IHME_Region_NameSouthern Sub-Saharan Africa')] <- 2
...
abline(h = 0, lty = 2)
abline(h = mean(reg_coeff_2[!(names(reg_coeff_2) %in% c('IHME_Region_NameAndean Latin America', 'IHME_Region_NameCentral Asia', 'IHME_Region_NameSoutheast Asia', 'IHME_Region_NameEastern Sub-Saharan Africa', 'IHME_Region_NameSouth Asia', 'IHME_Region_NameSouthern Sub-Saharan Africa'))]), col = 2)
