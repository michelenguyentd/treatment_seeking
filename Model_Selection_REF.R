...
model_no <- "RF_" # 1. RF (random effect for region and units)
...
train_data_HMISfrac$Time_Factor_2[!(train_data_HMISfrac$IHME_Region_Name %in% c("Central Latin America", 'Southeast Asia', "Western Sub-Saharan Africa",  "Eastern Sub-Saharan Africa", "Southern Sub-Saharan Africa"))] <- 'Other'
...
test_data_HMISfrac$Time_Factor_2[!(test_data_HMISfrac$IHME_Region_Name %in% c("Central Latin America", 'Southeast Asia', "Western Sub-Saharan Africa",  "Eastern Sub-Saharan Africa", "Southern Sub-Saharan Africa"))] <- 'Other'
...
for (i in 1:length(cov_ind)){
  # if (!(i %in% c(12, 19))){ # Singular convergence.
    var_values_Any <- train_data_Any[, cov_ind[i]]
    gam_Any <- gamm(logit_Any ~ s(Year, by = Time_Factor) + s(X), data = data.frame("logit_Any" = train_data_Any$logit_Any, "X" = var_values_Any, 'Year' = train_data_Any$Year, 'Time_Factor' = train_data_Any$Time_Factor, 'IHME_Region_Name' = train_data_Any$IHME_Region_Name, 'IHME_location_id' = train_data_Any$IHME_location_id), random = list(IHME_Region_Name = ~ 1, IHME_location_id = ~ 1))
    AIC_Any[i] <- AIC(gam_Any$lme)
  # }
  # if (!(i %in% c(3))){ # Singular convergence.
  var_values_HMIS <- train_data_HMISfrac[, cov_ind[i]]
  gam_HMIS <- gamm(logit_HMIS ~ s(Year, by = Time_Factor_2) + s(X), data =  data.frame("logit_HMIS" = train_data_HMISfrac$logit_HMIS, "X" = var_values_HMIS, 'Year' = train_data_HMISfrac$Year, 'Time_Factor_2' = train_data_HMISfrac$Time_Factor_2, 'IHME_Region_Name' = train_data_HMISfrac$IHME_Region_Name, 'IHME_location_id' = train_data_HMISfrac$IHME_location_id), random = list(IHME_Region_Name = ~ 1, IHME_location_id = ~ 1))
...
formula_any <-  logit_Any ~ s(Year, by = Time_Factor, k = 5) + s(ANC1_coverage_prop, k = 3) + s(GDPpc_id_b2010, k = 3) + s(DMSP_nighttime, k = 3) + s(hospital_beds_per1000, k = 3) 
test.model <- gamm(formula_any, data = train_data_Any, random = list(IHME_Region_Name = ~ 1, IHME_location_id = ~ 1))

summary(test.model$gam)

model_any <- uGamm(logit_Any ~ s(Year), data = train_data_Any, random = list(IHME_Region_Name = ~ 1, IHME_location_id = ~ 1))

model_any$gam$formula <- formula_any

temptime <- proc.time()[3]
model.select.any <- dredge(model_any, fixed=c("s(Year, by = Time_Factor, k = 5)"), m.min=4)
timetaken <- proc.time()[3] - temptime  # About 3.35 mins.
model.select.any[1:10]

formula_HMIS <-  logit_HMIS ~ s(Year, by = Time_Factor_2, k = 5) + s(ANC1_coverage_prop, k = 3)  + s(VIIRS_nighttime, k = 3) + s(measles_vacc_cov_prop, k = 3) + s(frac_oop_hexp, k = 3) + s(prop_urban, k = 3) + s(hospital_beds_per1000, k = 3) + s(log_the_pc, k = 3)

test.model <- gamm(formula_HMIS, data = train_data_HMISfrac, random = list(IHME_Region_Name = ~ 1, IHME_location_id = ~ 1))

summary(test.model$gam)

model_HMIS <- uGamm(logit_HMIS ~ s(Year), data = train_data_HMISfrac, random = list(IHME_Region_Name = ~ 1, IHME_location_id = ~ 1))

model_HMIS$gam$formula <- formula_HMIS

temptime2 <- proc.time()[3]
model.select.HMIS <- dredge(model_HMIS, fixed=c('s(Year, by = Time_Factor_2, k = 5)'), m.min=4)
...
formula_any_1 <-  logit_Any ~ s(Year, by = Time_Factor, k = 5) + s(ANC1_coverage_prop, k = 3)  + s(DMSP_nighttime, k = 3) + s(GDPpc_id_b2010, k = 3)

model_any_1 <- gamm(formula_any_1, data = train_data_Any, random = list(IHME_Region_Name = ~ 1, IHME_location_id = ~ 1))

refRegion <- ranef(model_any_1$lme)

pdf(paste(graphics.path, model_no, "Any_model_REF_QQ.pdf", sep = ""), width = 8, height = 4)
par(mfrow = c(1, 2))
qqnorm(refRegion$IHME_Region_Name[, ], main = 'Normal Q-Q plot (Region)')
qqline(refRegion$IHME_Region_Name[, ]) 
# Regional random effects skewed -> use fixed effects instead. 
qqnorm(refRegion$IHME_location_id[, ], main = 'Normal Q-Q plot (Unit)')
qqline(refRegion$IHME_location_id[, ])
# Unit random effects OK apart from a few outliers. 
dev.off()

hist(refRegion$IHME_Region_Name[, ])

# To be changed for different models:

gamtabs(model_any_1$gam, caption = "Summary of best any TS model with random effects for the regions and units.")
...
formula_hmis_1 <- logit_HMIS ~ s(Year, by = Time_Factor_2, k = 5) + s(ANC1_coverage_prop, k = 3) + s(hospital_beds_per1000, k = 3) + s(log_the_pc, k = 3) +s(frac_oop_hexp, k = 3) + s(measles_vacc_cov_prop, k = 3) + s(prop_urban, k = 3) + s(VIIRS_nighttime, k = 3) 

model_hmis_1 <- gamm(formula_hmis_1, data = train_data_HMISfrac, random = list(IHME_Region_Name = ~ 1, IHME_location_id = ~ 1))
refRegion_HMIS <- ranef(model_hmis_1$lme)

pdf(paste(graphics.path, model_no, "HMIS_model_REF_QQ.pdf", sep = ""), width = 8, height = 4)
par(mfrow = c(1, 2))
qqnorm(refRegion_HMIS$IHME_Region_Name[, ], main = 'Normal Q-Q plot (Region)')
qqline(refRegion_HMIS$IHME_Region_Name[, ]) 
qqnorm(refRegion_HMIS$IHME_location_id[, ], main = 'Normal Q-Q plot (Unit)')
qqline(refRegion_HMIS$IHME_location_id[, ])
dev.off()

hist(refRegion_HMIS$IHME_Region_Name[, ])


gamtabs(model_hmis_1$gam, caption = "Summary of best public fraction model with random effects for the regions and units.")
...
plot(model_hmis_1$gam, select = 13, ylim = c(-5, 5))
dev.off()
